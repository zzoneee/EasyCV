...
class Distill(BaseModel):
    ...
    def __init__(self,
        ...
        self.teacher_model = builder.build_backbone(teacher_model)

        self.teacher_model_url = teacher_model_url
    ...
    def forward_train(self, img, gt_labels) -> Dict[str, torch.Tensor]:
        ...
        with torch.no_grad():
            teacher_model_outs = self.teacher_model(img)
            teacher_model_outs_detach = teacher_model_outs[0]
        ...
        for idx in range(self.head_num):
            ...
            loss_inputs = (outs, gt_labels, teacher_model_outs_detach)
            hlosses = h.distill_loss(*loss_inputs)
